...
class AnswerExtractionOperator(OperatorABC):
    def __init__(self, llm_serving: Union[None, object] = None, overwrite: bool = False):
        ...
        self.system_prompt = "You are a professional question answering system. Extract concise and accurate answers from the provided solutions. Output only the answer without any additional text."
    ...
    def get_desc(lang: str = "zh"):
        if lang == "zh":
            ...
            return (
                ...
                "Input Parameters:\n"
                "- input_key: Solution field name, default 'solution'\n"
    ...
    def run(self, storage: DataFlowStorage, input_key: str = "solution", output_key: str = "answer"):
        dataframe = storage.read("dataframe")

        if input_key not in dataframe.columns:
            raise ValueError(f"input_key: {input_key} not found in dataframe columns.")
        ...
        mask = dataframe[input_key].apply(lambda x: _is_valid(x, empty_ok=False))
        ...
        # 收集有效 solutions
        solutions = dataframe.loc[mask, input_key].tolist()
